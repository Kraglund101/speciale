"""
Dilate AFTER crop+resize to 224 with random scale S from 1024.

S_min = max(224, bbox_side) — guarantees full anomaly always in crop.

For each sample, shows valid crop sizes with:
...
def load_samples(splits_dir, data_root, n=200, product_filter=None):
    samples = []
...
def get_bbox_info(mask_np):
    ...
    if len(ys) == 0:
        h, w = mask_np.shape
        return (h // 2, w // 2), 0, 0
    # Bbox center (NOT centroid) — guarantees all blobs fit in crop
...
def main():
    ...
    save_dir = root / "results" / "temp_dilate_after_crop_mint"
    save_dir.mkdir(parents=True, exist_ok=True)

    samples = load_samples(splits_dir, data_root, n=999, product_filter="mint")
    ...
    for i, sample in enumerate(samples):
        ...
        mask_pil = Image.fromarray(((mask_raw > 127).astype(np.uint8) * 255), mode="L")
        ...
        center, bbox, area_1024 = get_bbox_info(mask_t.squeeze(0).numpy())

        # S_min = max(224, bbox_side) — full anomaly must fit in crop
        s_min = max(224, bbox)
        # S_max = 512 — beyond this, 224 resize is too destructive
        s_max = 512

        if s_min > s_max:
            # bbox > 512, use bbox as both min and max (single crop size)
            s_max = s_min
        ...
        # Column 0: original 1024 image (shown at display size) with mask overlay
        orig_np = img_t.permute(1, 2, 0).numpy()
        orig_mask_np = mask_t.squeeze(0).numpy()
        pct_1024 = area_1024 / (1024 * 1024) * 100
        draw_overlay(axes[0, 0], orig_np, orig_mask_np,
                     np.zeros_like(orig_mask_np),
                     f"Original 1024x1024\n"
                     f"area={area_1024}px ({pct_1024:.2f}%)\n"
                     f"bbox={bbox}px")
        # Row 1, col 0: original with binary mask overlay
        ...
        axes[1, 0].set_title(f"Mask @1024\n{area_1024}px ({pct_1024:.2f}%)", fontsize=8)
        axes[1, 0].axis("off")

        for j, cs in enumerate(crop_sizes):
            col = j + 1  # offset by 1 for original column
        ...
        plt.suptitle(
            f"Sample {i} \u2014 {atype} \u2014 bbox={bbox}px, area={area_1024}px @1024\n"
            f"S in [{s_min}, {s_max}]. Bbox-centered crop, max 512 to avoid destructive downsample.",
            fontsize=11, fontweight="bold",
        )
        plt.tight_layout(rect=[0.08, 0.04, 1, 0.91])
        plt.savefig(save_dir / f"{i:02d}_{atype}.png", dpi=120, bbox_inches="tight")
        plt.close()
        print(f"  [{i+1}/{len(samples)}] {atype} (bbox={bbox}, S in [{s_min},{s_max}])")
